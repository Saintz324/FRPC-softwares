...
function Dial({ mounted }: { mounted: boolean }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${size} ${size}`} style={{ maxWidth: 380 }} suppressHydrationWarning>
      ...
      <text x={cx - 130} y={cy - 110} fill="rgba(255,255,255,0.55)" fontSize="11" fontFamily="var(--font-mono)">Uptime</text>
      ...
      <text x={cx} y={cy + 38} textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize="11" fontFamily="var(--font-mono)">Produtos Ativos</text>
    </svg>
  )
}

const PRODUCTS = [
  { label: 'Calendário de Férias', status: 'Ativo', href: '/produtos/calendario-de-ferias', color: 'rgba(59,130,246,0.15)', dot: '#60a5fa' },
  { label: 'Project Manager',      status: 'Ativo', href: '/produtos/project-manager',      color: 'rgba(52,211,153,0.12)', dot: '#34d399' },
  { label: 'Portal FRPC',          status: 'Em breve', href: '/pricing',                   color: 'rgba(139,92,246,0.10)', dot: '#a78bfa' },
]
...
export function WorkspaceSection() {
  ...
  const chips: [string, boolean][] = [
    ['Calendário de Férias', false], ['Project Manager', false], ['Portal FRPC', true],
    ['Cloud Native', false], ['Suporte Prioritário', false], ['Feito em Portugal', false],
  ...
  return (
    <div style={{ position: 'relative', background: '#050506', minHeight: '100dvh', padding: '80px 0 120px', overflow: 'hidden', color: 'var(--ink)' }}>
      ...
      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1, padding: '0 20px 24px' }}>
        <h2 className="display" style={{ fontSize: 'clamp(48px, 6vw, 84px)', margin: '0 0 12px' }}>Os nossos Produtos</h2>
        <p style={{ color: 'var(--ink-dim)', maxWidth: 540, margin: '0 auto 22px', fontSize: 14 }}>
          Software feito em Portugal para simplificar o trabalho das equipas — do calendário de férias à gestão de projetos.
        </p>
        <Link href="/pricing">
          <button className="btn btn-ghost" style={{ padding: '10px 18px', fontSize: 13 }}>Ver preços</button>
        </Link>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'center', padding: '36px 56px', position: 'relative', zIndex: 1, maxWidth: 1280, margin: '0 auto' }} className="ws-grid">
        <div>
          <div style={{ fontSize: 12, color: 'var(--ink-mute)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>Produtos em produção</div>
          <div style={{ fontSize: 'clamp(64px, 7vw, 92px)', fontFamily: 'var(--font-display)', letterSpacing: '-0.02em', lineHeight: 1, marginBottom: 28 }}>2 Ativos</div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 380 }}>
            {PRODUCTS.map((p) => (
              <Link key={p.label} href={p.href} style={{ textDecoration: 'none' }}>
                <div className="card" style={{ padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, background: p.color, cursor: 'pointer', transition: 'border-color 0.2s' }}>
                  <div>
                  ...
                  <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: p.dot, boxShadow: `0 0 8px ${p.dot}`, flexShrink: 0 }} />
          ...
          <div className="row gap-8" style={{ marginTop: 24 }}>
            <Link href="/produtos/calendario-de-ferias">
              <button className="btn btn-light" style={{ padding: '8px 18px' }}>Ver Produtos</button>
            </Link>
            <Link href="/pricing">
              <button className="btn btn-ghost" style={{ padding: '8px 18px' }}>Preços</button>
            </Link>
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Dial mounted={mounted} /></div>
      </div>

      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', padding: '0 32px 24px', position: 'relative', zIndex: 1 }}>
        {chips.map(([label, on]) => (
          <span key={label} className="chip" style={on ? { background: '#f4f4f1', color: '#0a0a0a', borderColor: '#f4f4f1' } : {}}>{label}</span>
        ))}
      </div>
